...
func (suite *EventDispatcherTestSuite) TestEventDispatcher_Register_WhenSameHandler() {
	err := suite.eventDispatcher.Register(suite.event.GetName(), &suite.handler)
	suite.Nil(err)
	suite.Equal(1, len(suite.eventDispatcher.handlers[suite.event.GetName()]))
...
func (suite *EventDispatcherTestSuite) TestEventDispatcher_Dispatch() {
	eventHandlerMock := &MockEventHandler{}
	eventHandlerMock.On("Handle", &suite.event, mock.Anything)

	eventHandlerMock2 := &MockEventHandler{}
	eventHandlerMock2.On("Handle", &suite.event, mock.Anything)

	err := suite.eventDispatcher.Register(suite.event.GetName(), eventHandlerMock)
	suite.Nil(err)
	err = suite.eventDispatcher.Register(suite.event.GetName(), eventHandlerMock2)
	suite.Nil(err)

	err = suite.eventDispatcher.Dispacth(&suite.event)
	suite.Nil(err)

	eventHandlerMock.AssertExpectations(suite.T())
	eventHandlerMock2.AssertExpectations(suite.T())
	eventHandlerMock.AssertNumberOfCalls(suite.T(), "Handle", 1)
	eventHandlerMock2.AssertNumberOfCalls(suite.T(), "Handle", 1)
}
